 ...
 * Description: Populate your site with random generated content. You can configure the post type, description, tags, post meta, terms, images, publish date, status, parent, sticky, etc.
 * Author: Iulia Cazan
 * Version: 3.0
 * Author URI: https://profiles.wordpress.org/iulia-cazan
 * License: GPL2
 *
 * Copyright (C) 2015-2016 Iulia Cazan
 *
...
class SISANU_Popupate_Posts
{
	...
	public static function get_plugin_settings() {
		...
		self::$default_settings = array(
			...
			'specific_date'=> '',
			'specific_status' => '',
			'initial_images' => plugins_url( '/assets/images/sample1.jpg', __FILE__ ) . chr( 13 ) . plugins_url( '/assets/images/sample2.jpg', __FILE__ ) . chr( 13 ) . plugins_url( '/assets/images/sample3.jpg', __FILE__ ),
			'images_list' => '',
			'images_path' => '',
	...
	public static function populate_posts_settings() {
		...
		<p>
			<?php _e( 'The is a helper plugin that allows developers to populate the sites with <strong>random content</strong> (posts with random tags, images, date in the past or in the future, sticky flag), but with specific meta values and taxonomy terms associated if the case.', 'spp' ); ?>
		</p>
		...
		<form id="spp_settings_frm" action="" method="post" class="spp">
			...
			<table class="spp-table wp-list-table widefat fixed posts" width="100%">
				<tr>
					...
					<td>
						...
						<input type="text" name="spp[tags_list]" id="spp_tags_list" value="<?php echo esc_attr( self::$settings['tags_list'] ); ?>" size="20"/>
						<br/><?php esc_html_e( 'separated by comma', 'spp' ); ?>
					</td>
					<td>
				...
				<tr>
					<td><h3><?php esc_attr_e( 'Post', 'spp' ); ?></h3></td>
					<td>
						<?php _e( 'Maximum', 'spp' ); ?><br>
					...
					<td>
						...
						<select name="spp[post_type]" id="spp_post_type">
							<?php if ( ! empty( self::$allowed_post_types ) ) :
								foreach ( self::$allowed_post_types as $k => $v ) : ?>
					...
					<td>
						<div id="spp_specific_date_wrap" <?php if ( 3 != self::$settings['date_type'] ) { echo ' style="display:none;"';}?>>
							<?php esc_html_e( 'Specific Date', 'spp' ); ?><br>
							<input type="text" name="spp[specific_date]" id="spp_specific_date" value="<?php esc_attr_e( self::$settings['specific_date'] ); ?>" size="15" placeholder="<?php echo esc_attr( date( 'Y-m-d H:i:s' ) ); ?>"/><em>yyyy-mm-dd hh:ii:ss</em>
							<hr>
							...
							<select name="spp[specific_status]" id="spp_specific_status">
								...
								<?php if ( ! empty( self::$allowed_post_statuses ) ) :
									foreach ( self::$allowed_post_statuses as $k => $v ) : ?>
					...
					<td>
						...
						<select name="spp[taxonomy]" id="spp_taxonomy">
							<?php if ( ! empty( self::$allowed_taxonomies ) ) :
								foreach ( self::$allowed_taxonomies as $k => $v ) : ?>
									<option value="<?php echo esc_attr( $k ); ?>"<?php selected( $k, self::$settings['taxonomy'] ); ?>><?php echo esc_attr( $v ); ?></option>
							<?php endforeach;
							endif; ?>
						</select><br>
						<input type="text" name="spp[term_id]" id="spp_term_id" value="<?php echo esc_attr( self::$settings['term_id'] ); ?>" size="10" placeholder="<?php esc_attr_e( 'term_id', 'spp' ); ?>"/>
					</td>
					<td>
						...
						<input type="text" name="spp[term_id2]" id="spp_term_id2" value="<?php echo esc_attr( self::$settings['term_id2'] ); ?>" size="10" placeholder="<?php esc_attr_e( 'term_id', 'spp' ); ?>"/>
					</td>
					<td>
						<?php esc_html_e( 'Post Meta', 'spp' ); ?><br>
						<input type="text" name="spp[meta_key]" id="spp_meta_key" value="<?php echo esc_attr( self::$settings['meta_key'] ); ?>" size="15" placeholder="<?php esc_attr_e( 'meta_key', 'spp' ); ?>"/><br>
						<input type="text" name="spp[meta_value]" id="spp_meta_value" value="<?php echo esc_attr( self::$settings['meta_value'] ); ?>" size="10" placeholder="<?php esc_attr_e( 'meta_value', 'spp' ); ?>"/>
					</td>
					<td>
						<?php esc_html_e( 'Post Meta 2', 'spp' ); ?><br>
						<input type="text" name="spp[meta_key2]" id="spp_meta_key2" value="<?php echo esc_attr( self::$settings['meta_key2'] ); ?>" size="15" placeholder="<?php esc_attr_e( 'meta_key', 'spp' ); ?>"/><br>
						<input type="text" name="spp[meta_value2]" id="spp_meta_value2" value="<?php echo esc_attr( self::$settings['meta_value2'] ); ?>" size="10" placeholder="<?php esc_attr_e( 'meta_value', 'spp' ); ?>"/>
					</td>
					<td>
						<?php esc_html_e( 'Post Meta 3', 'spp' ); ?><br>
						<input type="text" name="spp[meta_key3]" id="spp_meta_key3" value="<?php echo esc_attr( self::$settings['meta_key3'] ); ?>" size="15" placeholder="<?php esc_attr_e( 'meta_key', 'spp' ); ?>"/><br>
						<input type="text" name="spp[meta_value3]" id="spp_meta_value2" value="<?php echo esc_attr( self::$settings['meta_value3'] ); ?>" size="10" placeholder="<?php esc_attr_e( 'meta_value', 'spp' ); ?>"/>
					</td>
				</tr>
				...
				<tr>
					...
					<td colspan="3">
						<textarea name="spp[images_list]" id="spp_images_list" cols="20" rows="2"></textarea><br>
						<?php esc_html_e( 'Click here to ', 'spp' ); ?> <a onclick="jQuery('#spp_images_list').val( jQuery('#spp_initial_images').html() );"><?php esc_html_e( 'use the plugin sample images', 'spp' ); ?></a>. <?php esc_html_e( 'You can add your own images URLs and click the "save settings" button (add URLs of images, separated by new line).', 'spp' ); ?>
									<span id="spp_initial_images"><?php echo self::$settings['default_images']; ?></span>
					</td>
					<td colspan="2">
			...
			?>
			<table id="spp-will-cleanup" class="<?php echo esc_attr( $class ); ?>">
				<tr>
					<td><input type="checkbox" name="spp[cleanup_on_deactivate]" id="spp_cleanup_on_deactivate" <?php checked( self::$settings['cleanup_on_deactivate'], 1 ); ?> onclick="jQuery('#spp-will-cleanup').toggleClass('spp-will-cleanup');">
					</td>
					<td><?php esc_html_e( 'Cleanup on deacvtivate', 'spp' ); ?> <em>(<?php esc_html_e( 'please be careful, if you enable this option, when you deactivate the plugin, the content populated with this plugin will be removed, including images', 'spp' ); ?>)</em>.</td>
				</tr>
			</table>
			...
			<tr>
				<td class="vtopAlign">
					<div class="floatright textright">
						<span class="dashicons dashicons-smiley"></span> <b><?php esc_html_e( 'I don\'t mind if you donate !', 'spp' ); ?></b>
	...
	public static function get_random_description( $text_elements, $max_blocks = 1 ) {
	...
	public static function execute_add_random_posts() {
		...
		for ( $i = 0; $i < self::$settings['max_number']; $i ++ ) {
			...
			if ( ! empty( $name ) ) {
				$max_blocks = ( self::$settings['content_p'] == 0 ) ? mt_rand( 1, 6 ) : self::$settings['content_p'];
				...
				if ( ! empty( self::$settings['specific_date'] ) && self::spp_is_valide_date( self::$settings['specific_date'] ) ) {
					...
					$now_pref = - 1;
					if ( self::$settings['date_type'] == 2 ) {
						$now_pref = 1;
					} elseif ( self::$settings['date_type'] == 0 ) {
			...
			if ( ! empty( $new_file_content ) ) {
				...
				if ( empty( $upload['error'] ) ) {
					...
					if ( ! empty( $attach_id ) ) {
						...
						update_post_meta( $attach_id, 'spp_sample_url', md5( $file_url ) );
					}
				}
			}
	...
	public static function deactivate_plugin() {
		...
		if ( file_exists(  self::$settings['legacy_images_path'] ) && is_dir( self::$settings['legacy_images_path'] ) ) {
